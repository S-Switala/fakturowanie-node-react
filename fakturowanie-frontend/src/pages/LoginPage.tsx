...
export default function LoginPage() {
  ...
  // - czyścimy flagę anty-pętli
  // - lekki warmup (nie blokuje UI)
  useEffect(() => {
    ...
    if (sessionStorage.getItem(WARMED_RELOADED)) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // jeśli wygląda na zimny start — budzimy agresywniej i robimy auto-odświeżenie
      if (isColdStart(err)) {
        setMsg('Serwer się budzi… odświeżę stronę automatycznie')
        const ok = await warmup(6)
        if (ok) {
          // UWAGA: w produkcji możesz NIE zapisywać hasła
          sessionStorage.setItem(PRELOGIN_KEY, JSON.stringify({ email, password }))
          sessionStorage.setItem(WARMED_RELOADED, '1')
          setTimeout(() => {
            window.location.replace(window.location.href) // bez zaśmiecania historii
          }, 350)
          return
        }
      }
