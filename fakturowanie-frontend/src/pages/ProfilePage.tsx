...
type ProfileForm = Omit<RegisterFormData, 'password'> // profil nie zmienia hasła
...
export default function ProfilePage() {
  ...
  // Wymagane pola dla kontekstu "profile" (email/hasło NIE są wymagane)
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Backend może zwrócić brakujące klucze — uzupełniamy pustymi stringami
        ...
        // wstępna walidacja (na podstawie załadowanych danych)
        const initErrs = validateAll(data, required, optionalFields)
        setErrors(initErrs)
      } catch (err) {
  ...
  const handleChange = (field: keyof ProfileForm, value: string) => {
    ...
    // walidacja onChange dla zmienianego pola
    // Mapujemy pole do nazwy w validators (ta sama nazwa klucza)
    setErrors(prev => ({
      ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.put(ENDPOINTS.me, user, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  return (
    <label className="block mb-1 text-sm font-medium text-left flex items-center gap-1">
      ...
      {required &&
        (valid ? (
          <HiCheck className="text-green-600 inline w-4 h-4" />
        ) : (
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-2xl mx-auto p-6 mt-6 bg-white rounded shadow">
        ...
        <div className="grid gap-4">
          ...
          {/* Email – tylko do odczytu, bez gwiazdki */}
          <div>
